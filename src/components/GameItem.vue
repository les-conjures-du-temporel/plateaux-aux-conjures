  ...
  <q-card flat class="q-my-sm">
    <q-card-section horizontal>
      ...
      <q-card-section class="col-9">
        ...
        <div class="row">
          ...
          <div class="col-6">
            <q-btn color="white" text-color="black" :label="showDetails ? 'Masquer détails' : 'Voir détail'" size="sm"
                   outline class="q-my-xs"
                   no-caps
    ...
    <q-card-section v-if="showDetails" class="q-pa-xs">
      <span v-if="game.bgg.categories.length">
        <span class="text-bold">Catégories</span>: {{ game.bgg.categories.join(', ')
        }}<br />
      </span>
      <span v-if="game.bgg.mechanics.length">
          <span class="text-bold">Mécaniques</span>: {{ game.bgg.mechanics.join(', ')
        }}<br />
        </span>
      ...
      <span v-if="game.bgg.averageRating">
          <span class="text-bold">Note</span>: {{ formatNumber(game.bgg.averageRating) }} /
          10<br />
        </span>
      <span v-if="game.bgg.averageWeight">
          <span class="text-bold">Complexité</span>:
          {{ formatNumber(game.bgg.averageWeight) }} / 5<br />
        </span>
      <span class="text-bold">Code conjuré</span>: {{ game.clubCode }}<br />
      <span v-if="game.lastPlayed">
        <span class="text-bold">Parties enregistrées</span>: {{ game.totalPlays }}<br />
        <span class="text-bold">Dernière partie</span>: {{ formatDate(game.lastPlayed) }}
      </span>
    </q-card-section>
  </q-card>
</template>
